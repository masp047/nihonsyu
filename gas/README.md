...
`tick()` は現在時刻を見て、**08:50 / 15:50 になった“その分”だけ**投稿します。
これにより、GAS日次トリガーの「前後15分の幅」を避け、ほぼ指定どおりの時刻に投稿できます。

| 現在時刻 | 動作 |
|---|---|
| 08:50 ちょうど | A列が `08:50` の有効行を投稿 |
| 15:50 ちょうど | A列が `15:50` の有効行を投稿 |
| それ以外の分 | 何もしない（シートも読まない） |

- 同じ日に同じ時刻を二重投稿しないよう、実行済みフラグ（スクリプトプロパティ `LASTRUN_*`）で防止します。
...
- 投稿時刻を変えたい場合は、コード先頭 `CONFIG.SLOTS` の値とスプレッドシートA列を
  合わせて変更してください（`setupTriggers()` の再実行は不要です）。

> 毎分トリガーは1日1440回動きますが、該当時刻以外は即終了（シートも読まない）ため軽量で、
> Apps Script の実行時間の上限内で問題なく運用できます。
